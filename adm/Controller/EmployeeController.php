...
include_once $_SERVER['DOCUMENT_ROOT'] . "/adm/Model/Employee.php";
...
class EmployeeController extends Controller
{
    ...
    {
        $franchise_idx = !empty($params['center_idx']) ? $params['center_idx'] : '1';
        $state = !empty($params['state']) ? $params['state'] : '00';

        try {
            $result['memberData'] = $this->employeeInfo->loadEmployee($franchise_idx, $state);
            $selPosition = $this->employeeInfo->loadState($franchise_idx);

            $positionList = "<option value=''>선택</option>";
            if (!empty($selPosition)) {
                foreach ($selPosition as $key => $val) {
                    $positionList .= "<option value=\"" . $val['code_num2'] . "\">" . $val['code_name'] . "</option>";
                }
                $result['selPosition'] = $positionList;
            }
    ...
    public function selectEmployee($params)
    {
        ...
        try {
            $result = $this->employeeInfo->selectEmployee($user_no, $franchise_idx);
    ...
    public function searchId($params)
    {
        ...
        try {
            $result = $this->employeeInfo->searchId($user_name, $franchise_idx);
            $table = '';
