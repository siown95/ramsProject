...
include_once $_SERVER['DOCUMENT_ROOT'] . "/adm/Model/ReceiptItem.php";
...
class ReceiptItemController extends Controller
{
        ...
        try {
            ...
            if (!empty($result)) {
                foreach($result as $key => $val) {
                    $result[$key]['receipt_amount'] = number_format($result[$key]['receipt_amount']);
                }
            }
            return $result;
        } catch (Exception $e) {
    ...
    public function receiptItemBatch($request)
    {
        ...
        try {
            if (empty($sel_center) || $sel_center == '1') {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }

            if ($sel_center == 'all') {
                $this->receiptItemInfo->where_qry = " franchise_idx <> '1' AND headYn = 'Y' ";
            } else {
