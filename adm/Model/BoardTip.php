...
class BoardTipInfo extends Model
{
    ...
    {
        $sql = "DELETE FROM {$this->likes_table} WHERE board_idx = '" . $board_idx . "'";
    ...
    {
        $sql = "SELECT likes_idx FROM {$this->likes_table} WHERE board_idx = '" . $board_idx . "' AND franchise_idx = '1' AND user_no = '" . $user_no . "'";
        $result = $this->db->sqlRow($sql);
