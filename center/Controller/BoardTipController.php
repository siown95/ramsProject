...
include_once $_SERVER['DOCUMENT_ROOT'] . "/center/Model/BoardTip.php";
...
class BoardTipController extends Controller
{
        ...
        try {
            ...
            if (!empty($result)) {
                foreach ($result as $key => $val) {
                    $myCmt = '';
                    $del_btn = '';
                    if ($_SESSION['logged_no'] == $val['cmt_user']) {
    ...
    public function cmtLikeChange($request)
    {
        ...
        try {
            if (empty($board_idx) || empty($user_no) || empty($franchise_idx)) {
                throw new Exception('필수값이 누락되었습니다.', 701);
            }

            $like_check = $this->boardTipInfo->checkLikeCnt($board_idx, $user_no, $franchise_idx);
