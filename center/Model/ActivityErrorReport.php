...
class ActivityErrorReportInfo extends Model
{
    ...
    {
        $sql = "SELECT AE.error_report_idx, AE.title, M.user_name, F.center_name, convert(varchar(19), AE.reg_date, 120) reg_date, AE.state
    ...
    {
        $sql = "SELECT AE.error_report_idx, AE.writer_no, AE.title, AE.contents, M.user_name, F.center_name, convert(varchar(19), AE.reg_date, 120) reg_date
                , AE.state, AE.file_name, AE.origin_name, AE.comments
                FROM {$this->table_name} AE
                LEFT OUTER JOIN member_centerm M
                ON AE.writer_no = M.user_no
                LEFT OUTER JOIN franchisem F
