...
class ReceiptItemtInfo extends Model
{
    ...
    {
        $sql = "SELECT RT.receipt_item_idx, C.code_name AS receipt_type, ISNULL(C2.code_name, '') AS lesson_type, ISNULL(C3.code_name, '') AS grade , RT.receipt_name, RT.receipt_amount
                ...
                WHERE RT.franchise_idx = '" . $center_idx . "' AND RT.useYn = 'Y'
    ...
    {
        $sql = "SELECT franchise_idx, receipt_type, receipt_lesson_type, receipt_grade, receipt_name, receipt_amount, useYn, headYn FROM {$this->table_name} WHERE receipt_item_idx = '" . $receiptItemIdx . "'";
    ...
    public function receiptItemBatch($franchise_idx)
    {
        $sql = "INSERT INTO {$this->table_name} (franchise_idx, receipt_type, receipt_lesson_type, receipt_grade, receipt_name, receipt_amount, headYn)
                ...
                WHERE F.useyn = 'Y' AND RT.useYn = 'Y' AND RT.franchise_idx = '1'  AND F.franchise_idx = '" . $franchise_idx . "')";

        $result = $this->db->execute($sql);
        return $result;
    }

    public function getReceiptList($center_idx, $grade)
    {
        $sql = "SELECT receipt_item_idx, receipt_name FROM receiptT WHERE franchise_idx = '{$center_idx}' AND receipt_grade = '{$grade}'  AND receipt_lesson_type = '02'";
        $result = $this->db->sqlRowArr($sql);
        return $result;
    }
}
