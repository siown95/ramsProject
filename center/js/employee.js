$(document).ready(function () {
    fnCheck();
    loadEmployee('00'); //직원리스트 호출

    $("#employeeTable").on('page.dt', function () {
        $("#employeeTable > tbody > tr").removeClass('bg-light');
...
function setFormdata(data) {
    ...
    $("#gender" + data.gender).attr('checked', true);
    ...
    if (data.position) {
        $("#position option[value=" + data.position + "]").prop("selected", true);
    } else {
