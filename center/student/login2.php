...
$center_name = $infoClassCmp->getFranchiseeInfo3($user_id);
...
if (!empty($idData['cnt'])) {
    ...
    $rs = $permissionCls->getPermission($user_id, $password, 'student', $centerIdx);
    ...
    if (!empty($user_data)) {
        if ($user_data['state'] == '99') {
            ...
            $returnMsg = '휴원 상태입니다. 관리자에게 문의 바랍니다.';
        } else if ($user_data['state'] == '02') {
            $returnMsg = '퇴원 상태입니다. 관리자에게 문의 바랍니다.';
        } else {
            $status = 'success';
...
$result_arr['center_name'] .= $center_name;
